defmodule Cabinet.Invoices do
  ...
  def with_virtual_fields(%Schema.Invoice{} = invoice) do
    ...
    subtotal = Enum.reduce(invoice.units, 0, fn %Schema.Unit{} = unit, acc ->
      unit.cost
      |> Decimal.mult(unit.count)
      |> Decimal.add(acc)
    end)

    total_gst = if invoice.gst do
    ...
    %{ invoice | late?: late?, days_overdue: due_diff, subtotal: subtotal, total_gst: total_gst, amount_due: amount_due }
  end
end
